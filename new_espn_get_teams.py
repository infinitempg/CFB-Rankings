...
def get_team(team):
    ...
    team_sch_table = soup.find_all('tbody',{'class':'Table2__tbody'})
    # team_sch_rows = team_sch_table.find_all('tr')
    team_sch_rows = team_sch_table[0].find_all('tr')
    ...
    for n in range(2,len(team_sch_rows)):
        try:
            ...
            row_data = team_sch_rows[n].find_all('span')
            ...
            opp_id = row_data[2].find('img').get('src').split('.')[-2].split('/')[-1]
            thisweek['opp_id'] = opp_id

            # opponent = team_sch_rows[n].find_all('li',{'class':'team-name'})
            # opp_name = team_sch_rows[3].find_all('img')[0].get('title')
            opp_name = row_data[3].text
            # opp_name = opponent[0].text

            if team_sch_rows[n].find_all('td')[-1].text == 'Canceled' or team_sch_rows[n].find_all('td')[-1].text == 'Postponed':
                continue
...
#             else: scores = score[0].text.strip(' ').strip(' OT').split('-')
            ...
            scores = row_data[5].text.split(' ')[0].split('-')

            # print(scores)

            # game_status = team_sch_rows[n].find_all('li',{'class':'game-status'})
            location_str = row_data[1].text
            if opp_name[-1] == '*':
                location = 'N'
            else:
                if location_str == "@":
                    location = 'A'
                elif location_str == 'vs':
                    location = 'H'

            thisweek['loc'] = location

            # result = game_status[1].string
            result = row_data[4].text
            ...
            record = row_data[6].text.split(' ')[0]
            wins = record.split('-')[0]
